class Ball:
    ...
    def draw(self):
        ...
        if self.pos_y > height-50 - self.sz/2: #if ball y position becomes greater than the height minus 50 minus half the ball size
            if self.paddle_hit_detection():     #if ball hits the paddle
                self.dy *= -1.3             #reverse ball y direction and increase by %30
                self.dx *= 1                # x direction of ball stays the same
        ...
        fill(65, 149, 242)                     #Ball fill colour is green
        square(self.pos_x, self.pos_y, self.sz) #makes a circle to represent the ball
        for i in range(self.lives):          #for the number of lives, draws balls at the top of the screen
            square(10+i * 20, 15, self.sz)   
            
    def paddle_hit_detection(self):
        if self.pos_x > self.padpos - 20 and self.pos_x < self.padpos + 20 and self.pos_y < height-49: #if ball position x is greater than the paddle poition minus 20, and is less than the paddle position plus 20 and the ball y position is greater than the height les 49(the height where the paddle is)
